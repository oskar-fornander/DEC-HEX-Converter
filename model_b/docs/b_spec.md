# DEC–HEX Converter, Model B

Electronics Kit

Looking for a sleek, physical device to quickly convert between decimal and hexadecimal values? This is the kit for you! You get the satisfaction of soldering and assembling it yourself, turning it into a rewarding and functional project.

* **Fast conversion** between decimal, hexadecimal, and binary numbers—in all directions.
* The **rotary encoder** provides responsive and precise tactile feedback, featuring distinct steps for each value increment/decrement.
* **Rotate** to count up/down, and **press** to reset the counter to zero.
* Values are displayed simultaneously in decimal, hexadecimal, and binary form using an **LCD (Liquid Crystal Display)**.
* **No power switch**; the unit automatically enters an **ultra-low-power sleep mode** after a period of inactivity. Wake the unit up by rotating the knob or pressing to reset.
* Powered by **4 AAA batteries**. Provides over **25 hours of active use**.
* The brain is an **ATtiny85/45 microcontroller**.
* Comes with a **3D-printed enclosure** (116mm x 82mm x 38mm).
* Delivered as a complete kit with carefully selected, high-quality components, a **pre-programmed microcontroller**, and a custom-designed **PCB**—making for simple assembly and excellent **soldering practice**. PCB prototypes for the current revision were kindly sponsored by PCBWay. Current kit use a slightly updated PCB revision with minor improvements compared to the version shown in some of the photos.
* The microcontroller (ATtiny85/45) is pre-programmed but can be **re-programmed**. The source code is available on GitHub.

*Read more in [**ordering.md**](../../ordering.md) for pricing and how to order your own kit.*

(Also check out Model A, which uses seven-segment displays instead of an LCD and involves slightly more soldering.)

/Oskar Fornander, 2025
